...
let HighlightNotificationDetialForm = props => {
	...
	const disabled = highlightNotification && highlightNotification.status !== 'pending';
				...
				<Grid.Row>
					<Grid.Column width={8}>
						<Segment>
							...
							<Form>
								{
									Boolean(highlightNotification) &&
									<Form.Group widths="equal">
										<Form.Field>
											<Form.Input
												value={LabelHelper.notificationStatus(highlightNotification.status)}
												disabled
											/>
											<label>Status</label>
										</Form.Field>
								...
								<Form.Group widths="equal">
									...
									<Form.Field>
										<Field
											...
											validate={[validateRequired]}
											disabled={disabled || highlightNotification}
										/>
										<label>Type</label>
									</Form.Field>
								</Form.Group>
								<Form.Group widths="equal">
									...
									<Form.Field>
										{/* <Field
											...
											options={OptionHelper.highlightNotificationRecipients()}
											validate={[validateRequired]}
											disabled={disabled}
										/>
					...
					<Grid.Column width={8}>
						<Grid>
							<Grid.Column
								...
								width={8}
							>
								{(!highlightNotification || (highlightNotification && highlightNotification.status === 'pending')) && (
									<Button
										content="Save"
										primary
										icon="save"
							...
							<Grid.Column
								...
								width={8}
							>
								{highlightNotification && highlightNotification.status === 'pending' && (
									<Button.Group>
										<Button
...
HighlightNotificationDetialForm = connect(
	state => ({
		...
		highlightNotification: state.highlightDetailPage.highlightNotificationDetailPageReducer.highlight_notification,
		initialValues: state.highlightDetailPage.highlightNotificationDetailPageReducer.highlight_notification
	})
)(HighlightNotificationDetialForm);
