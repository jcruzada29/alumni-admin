...
// Height 774, Width 337
// App Image dimension: 250px x 200px
const ALLOWED_WIDTH_RATIO = 250 * 4;
const ALLOWED_HEIGHT_RATIO = 200 * 4;

let PopupBasicForm = props => {
	...
	const optionsPopupsPublishStatus = CommonEnum.popup_publish_status.map(o => ({
		value: o,
		text: LabelHelper.popupPublishStatus(o)
	}));
										...
										<Form.Group widths="equal">
											{/* <Form.Field>
												<Field
													...
													style={{ minHeight: 300 }}
													name="description"
													placeholder="Popup Description (HTML)"
												/>
										...
										<Form.Group widths="equal">
											<Form.Field>
												<Field
													...
													clearable
													options={optionsPopupsPublishStatus}
													validate={[validateRequired]}
												/>
...
PopupBasicForm = reduxForm({
	form: 'PopupBasicForm'
})(PopupBasicForm);

const selector = formValueSelector('PopupBasicForm');
PopupBasicForm = connect(state => {
	const {
	...
	return {
		fileContent,
		fileContentName,
		initialValues: state.popupDetailPage.popupBasicPageReducer.popup
	};
})(PopupBasicForm);
