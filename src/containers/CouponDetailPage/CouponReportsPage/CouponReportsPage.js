...
class CouponReportsPage extends React.Component {
	...
	downloadXlsx = ({ csv, fileName }) => {
		...
		console.log(';test', bookingSummaryCsvRows);
	...
	confirmDownload = type => {
		...
		if (window.confirm('Are you sure you want to download?')) {
			this.props.downloadReports({ coupon_id: query.id, type });
			...
			<div className="bt-content-padded">
				<Segment style={{ height: '600px' }}>
					<Table singleLine>
						...
						<Table.Body>
							<Table.Row>
								<Table.Cell collapsing>All coupons</Table.Cell>
								<Table.Cell
									style={{ fontWeight: 'bold', color: '#0074bc' }}
									onClick={() => this.confirmDownload('all_coupons')}
								>
							...
							<Table.Row>
								...
								<Table.Cell
									style={{ fontWeight: 'bold', color: '#0074bc' }}
									onClick={() => this.confirmDownload('usage_history')}
								>
									<Icon name="cloud download" /> 
										Download
								</Table.Cell>
